...
- **#21 随机设备昵称（2026-09-06 新增，三次细化）**：互传/同步时双方显示可读随机昵称。**基座格式** `【形容词1】【形容词2】的【名词】`（如「软萌俏皮的樱桃」），词库三组（`NICK_ADJ1`/`NICK_ADJ2`/`NICK_NOUN`，各 50/50/100 词，与移动端 `device_nickname.dart` 同源，详见 `C:\cod\jianli\随即词库.md`）存于 `transferModule.ts`。**平台后缀规则**：本机展示（顶部栏 `src/components/header.vue` 经 IPC `transfer:nickname` 取 `currentNickname()`、两端「我的设备」卡片）只用基座、**不带任何后缀**；只有在「发现设备 / 最近设备」等广播给对端的场景才追加 `的PC`——`deviceInfo().name` 改为 `${currentNickname()}的PC`，故扫描到的对端名、最近设备、收发历史里的对端名都带 `的PC` 以便区分。（⚠️ 关键坑：PC 的发现广播来自 `syncModule.ts` 的 `deviceInfo()`，原先写死 `os.hostname()`，会导致移动端「发现设备」列表显示主机名 `DESKTOP-…` 而非昵称；已改为同样调用 `currentNickname()` 并加 `的PC` 后缀。`transferModule` 与 `syncModule` 互相 import `currentNickname`，仅运行时调用、无模块顶层依赖，打包器可安全处理循环依赖。）`currentNickname()` 从 `store._transfer_nickname`（`{name,ts}`）读取，**默认留存 3 天**（超过 `NICK_TTL` 才重随机）；**迁移**：旧格式（带 `的渐离App`/`的App`/`的PC`/` (PC)`/` (App)` 任一后缀）一律立即重生为新基座格式。`id` 仍用 `deviceId()` 主机名哈希，改名不影响唯一性。`transfer:status` 的 `name` 也回退为基座。改 `transferModule.ts` 必须重启 Electron。
...
- 主进程：
  - `electron/main/module/transfer/transferModule.ts`（单文件职责：发送客户端 + 接收端点 + 历史读写 + IPC 注册 + 事件推送；`initTransfer()` 入口；**改完必须重启 Electron**）
    ...
    - IPC：`transfer:status` / `transfer:scan`（复用 scanPeers）/ `transfer:pick-files`（dialog 仅选文件，`openFile,multiSelections`）/ `transfer:pick-folders`（dialog 仅选文件夹，`openDirectory,multiSelections`）—— 两者 #10 同返回原始选择项 `{path,name,isDir,size,fileCount}`，不拍平；**Windows 单对话框无法同时选文件+文件夹，故拆两个** / `transfer:send`（收 `peerIp` + 混合路径数组，主进程 `flattenSendPaths` 拍平）/ `transfer:cancel` / `transfer:history`（#20 支持 `{limit,offset}` 返 `{data,total}`）/ `transfer:open-received` / `transfer:open-file`（openPath 打开指定文件）/ `transfer:open-folder`（showItemInFolder 定位）/ `transfer:set-auto-accept` / `transfer:set-rename`（#9 rename/overwrite）/ `transfer:set-enc`（#14 加密开关）/ `transfer:recent-peers`（#11 读）/ `transfer:forget-peer`（#11 剔除）/ `transfer:answer-offer`（#15 `{tid,accept}`）/ `transfer:nickname`（#21 返回当前随机昵称，顶部栏展示用）
...
- 渲染端（不 import electron，全走 IPC）：
  - `src/views/fileTransfer/types.ts` —— TransferPeerDevice / TransferProgress / TransferHistoryItem / TransferStatus / LocalFileItem / **SelectedEntry**（选择列表项：path/name/isDir/size/fileCount/checked）
  - `src/views/fileTransfer/api/fileTransferApi.ts` —— 7 条 IPC 薄封装（`window.ipcRenderer.handlePromise`）
  - `src/store/useFileTransfer.ts` —— Pinia：status/peers/selectedDeviceIp/**entries**(SelectedEntry[] 选择列表)/**checkedCount/totalFiles/totalBytes/allChecked/someChecked**/files(发送中由 progress 重建的拍平列表)/progress(key=`${tid}|${fid}`)/history/autoAccept + scan/addManual/**pickFiles(累加去重·默认勾选)**/**toggleEntry/toggleAll/removeEntry/clearEntries**/send(只发已勾选项·文件夹由主进程拍平)/**cancel**/loadHistory/openReceived/**openFile/openFolder** + 批次聚合 `batchTotalBytes/batchSentBytes/batchPercent/batchRate/batchEta` + `bindEvents()`（订阅 `file-transfer:*` 事件）
  - `src/views/fileTransfer/index.vue` —— 页面薄壳（本机状态条 + 设备 + 发送面板 + 传输记录），onMounted `bindEvents`、onUnmounted `unbindEvents`
  - `src/views/fileTransfer/components/` —— `DeviceList.vue`（设备卡+发送，busy 时禁用）/ `TransferPanel.vue`（**选择文件/文件夹→`entries` 勾选列表（文件夹单独成行、多次累加、默认全选、可单条取消/移除/全选）**+逐文件 el-progress+**批次总进度条/速率/ETA**+自动接收开关+发送中「取消发送」）/ `TransferLog.vue`（历史列表，每条成功记录可「打开文件 / 打开所在文件夹」，失败显示 error 原因）
